...
window.clientScene = function(wsUrl, wsInstance, res) {
	...
	wsH["$+"] = function(obj) { //add kart. only used in debug circumstances. (people can't join normal gamemodes midgame)
		...
		if (t.mode != 1) return;
		var kart = new Kart([0, -2000, 0], 0, 0, obj.k.kart, obj.k.char, new ((obj.p)?((window.prompt("press y for cpu controlled") == "y")?controlRaceCPU:playerControls):controlNetwork)(t.activeScene.nkm, {}), t.activeScene);
	...
	function setUpCourse(mainNarc, texNarc, course, obj) {
		...
		for (var i=0; i<obj.k.length; i++) {
			...
			chars.push({charN:k.char, kartN:k.kart, controller:((pKart)?((window.prompt("press y for cpu controlled") == "y")?controlRaceCPU:playerControls):controlNetwork), raceCam:pKart, extraParams:[{k:"name", v:k.name}, {k:"active", v:k.active}]});
