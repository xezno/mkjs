...
window.singleScene = function(course, wsInstance, res) {
	...
	function setUpCourse(mainNarc, texNarc, course) {
		var chars = [];
		chars.push({charN:mchar, kartN:mkart, controller:((window.prompt("press y for cpu controlled") == "y")?controlRaceCPU:playerControls), raceCam:true, extraParams:[{k:"name", v:"single"}, {k:"active", v:true}]});
